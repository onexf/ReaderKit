...
/// settingsPanel 的固定内容高度（设置面板 + tab 栏）
///
/// settingsPanel 高度恒定、底部锚定在 bottomView 里，所以面板与 tab 栏的**绝对位置在展开与
/// 收起两种状态下完全一致**，展开只是 bottomView 长高后把面板露出来。
/// 之前 settingsPanel 高度随状态变化，面板会跟着容器顶边向上平移 166pt，横穿静止的 tab 栏
...
open class ReaderMenuBottomBar: ReaderMenuPanel {
    ...
    /// 进度
    public private(set) var progressPanel: ReaderMenuProgressPanel!
    
    /// 功能
    public private(set) var settingsPanel: ReaderMenuSettingsPanel!
    
    /// 目录视图
    public private(set) var cataloguePanel: ReaderMenuCataloguePanel!

    public override init(frame: CGRect) { super.init(frame: frame) }
    
    /// 获取当前应该显示的高度
    open func getCurrentHeight() -> CGFloat {
        if !cataloguePanel.isHidden {
            return READER_MENU_BOTTOM_VIEW_CATALOG_HEIGHT
        } else if settingsPanel.isOptionPanelShown() {
    ...
    open override func addSubviews() {
        ...
        progressPanel = ReaderMenuProgressPanel(hostMenu: hostMenu)
        progressPanel.isHidden = true
        addSubview(progressPanel)
        
        settingsPanel = ReaderMenuSettingsPanel(hostMenu: hostMenu)
        addSubview(settingsPanel)
        
        // 目录视图
        cataloguePanel = ReaderMenuCataloguePanel()
        cataloguePanel.isHidden = true
        cataloguePanel.onChapterChosen = { [weak self] chapterModel in
            self?.processCatalogChapterPicked(chapterModel)
        }
        addSubview(cataloguePanel)
    }
    
    /// 处理目录章节选中
    private func processCatalogChapterPicked(_ chapterModel: ReaderChapterListItemModel) {
        settingsPanel.processCatalogChapterPicked(chapterModel)
    }
    ...
    open override func layoutSubviews() {
        ...
        progressPanel.frame = CGRect(x: 0, y: 0, width: w, height: READER_MENU_PROGRESS_VIEW_HEIGHT)
        
        // settingsPanel 底部锚定、高度恒定：收起态时设置面板落在 bottomView 上方被裁掉，
        ...
        settingsPanel.frame = CGRect(x: 0, y: h - funcViewTotalHeight, width: w, height: funcViewTotalHeight)
        
        // 目录视图 - 从 0 开始到 tab 栏上方（目录态下面板不展开，按 tab 栏高度算）
        let catalogViewHeight = h - (READER_MENU_BOTTOM_TAB_BAR_HEIGHT + safeAreaBottom)
        cataloguePanel.frame = CGRect(x: 0, y: 0, width: w, height: max(0, catalogViewHeight))
    }
