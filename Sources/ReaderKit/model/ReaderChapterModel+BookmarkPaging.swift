...
//
//  核心约束:
//  - 临时分页(applyBookmarkPaging)只改内存态 pageModels/pageCount,绝不 save(),
...
extension ReaderChapterModel {
    ...
    public func coordinatedParagraphBegin(for location: Int) -> Int {
        guard let full = fullContent, full.length > 0 else { return max(0, location) }
    ...
    public func adoptBookmarkPaging(at location: Int) {
        ...
        // 确保常规分页已构建(fullContent 就绪)
        reviseFont()
        
        guard let full = fullContent, full.length > 0 else { return }
    ...
    /// fullContent 在临时分页时未被修改,直接整体重新分页即可恢复常规分页
    public func forceRebuildPaging() {
        guard let full = fullContent, full.length > 0 else {
            reviseFont()
            return
        }
        let rect = CGRect(origin: .zero, size: READER_VIEW_RECT.size)
        pageModels = ReaderTypesetter.pageing(attrString: full, rect: rect, isFirstChapter: false)
        pageCount = NSNumber(value: pageModels.count)
    }
    
    /// 滚动模式:计算 location 对应的 contentOffset.y(相对本章顶部)
    /// 累加 location 所在页之前各页 (contentSize.height + headTypeHeight),
    /// 再对所在页内 [页首, location) 子串做高度补偿。
    public func contentOffsetY(forLocation location: Int) -> CGFloat {
        guard let full = fullContent, full.length > 0 else { return 0 }
        ...
        for p in pageModels {
            ...
            if end <= target {
                // 整页在书签之前
                y += p.contentSize.height + p.headTypeHeight
    ...
    /// = headTypeHeight + 行顶相对内容区的 Y;供 scrollOffsetInPage 复用滚动控制器定位恢复。
    /// CTFrame 不可用时回退到子串测高。
    public func inPageOffsetY(forLocation location: Int) -> CGFloat {
        let p = page(location: location).intValue
        guard p >= 0, p < pageModels.count, let range = pageModels[p].range else { return 0 }
        let pageModel = pageModels[p]
        let headH = pageModel.headTypeHeight ?? 0
        ...
        // 回退:子串测高
        guard let full = fullContent, full.length > 0 else { return max(0, headH) }
        var y: CGFloat = headH
        if localIdx > 0 {
    ...
    public func location(forPage pageIndex: Int, inPageOffsetY offsetY: CGFloat) -> Int {
        guard let full = fullContent, full.length > 0,
              pageIndex >= 0, pageIndex < pageModels.count,
              let range = pageModels[pageIndex].range else { return 0 }
        ...
        if contentH > 0 {
            let frameRef = ReaderCoreText.makeFrame(attrString: pageModel.showContent,
    ...
    private func rowPeakY(inPage pageIndex: Int, localIndex: Int) -> CGFloat? {
        guard pageIndex >= 0, pageIndex < pageModels.count else { return nil }
        let pageModel = pageModels[pageIndex]
        let contentH = pageModel.contentSize.height
